...
def main():
    ...
    for line in infile:
        ...
        if re.findall(r'^whale.*whale$', line, re.IGNORECASE):
            line_count_begin_and_end += 1

    infile.close()

    print(f'\n{line_count_contain} lines in file contain target string.')
    print(f'{line_count_begin} lines in file contain target string at the beginning.')
    print(f'{line_count_end} lines in file contain target string at the end.')
    print(f'{line_count_begin_or_end} lines in file contain target string at the beginning or at the end.')
    print(f'{line_count_begin_and_end} lines in file contain target string at the beginning and at the end.')
