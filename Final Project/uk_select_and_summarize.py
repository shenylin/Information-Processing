"""
Select and summarizes qualifying U.K. expense records.

Prints report.
Writes summarized file.
"""

POUNDS_TO_DOLLARS_CONVERSION_FACTOR = 1.3114
...
def main():
    ...
    do_uk_select_and_summarize(target_year,
                               target_category,
                               data_directory,
                               input_filename,
                               output_filename)
...
def do_uk_select_and_summarize(target_year,
                               target_category,
                               data_directory,
                               input_filename,
                               output_filename):
...
def select_and_accumulate_qualifying_records(data_directory, input_filename, target_category, target_year):
    ...
    for line in infile:
        ...
        if line_count > 1:
            ...
            day, month, year = parse_uk_date(date)
            if year == target_year and category == target_category:
                qualifying_records += 1
                expense_accumulators[month - 1] += float(amount_as_string)*POUNDS_TO_DOLLARS_CONVERSION_FACTOR
    infile.close()
...
def parse_uk_date(uk_date):
    day_as_string, month_as_string, year_as_string = uk_date.split('/')
